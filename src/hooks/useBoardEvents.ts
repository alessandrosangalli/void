...
export function useBoardEvents() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (selectedNode) {
          ...
          if (selectedNode.type === 'text') {
            ...
            setConnections((prev) =>
              prev.filter(
                (c) =>
                  c.from.id !== selectedNode.id && c.to.id !== selectedNode.id,
              ),
            )
          } else if (selectedNode.type === 'image') {
            setImages((prev) =>
              prev.filter((img) => img.id !== selectedNode.id),
            )
            setConnections((prev) =>
              prev.filter(
                (c) =>
      ...
      if (key === 'f') setActiveTool('arrow')

      if (e.ctrlKey || e.metaKey) {
        const doZoom = (dir: 'in' | 'out' | 'reset') => {
          setCamera((p) =>
            zoomAtCenter(p, dir, window.innerWidth, window.innerHeight),
          )
        }
        if (key === '=' || key === '+') {
          e.preventDefault()
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent<SVGSVGElement>) => {
      ...
      if (activeTool === 'eraser' || e.target instanceof HTMLTextAreaElement)
        return
      ...
      } else if (activeTool === 'move' || activeTool === 'arrow') {
        setIsDragging(true)
        setLastPointer([e.clientX, e.clientY])
        setSelectedNode(null)
      }
  ...
  useEffect(() => {
    const handleWindowPointerMove = (e: PointerEvent) => {
      ...
      if (activeTool === 'draw' && e.buttons === 1) {
        ...
        (activeTool === 'move' || activeTool === 'arrow') &&
        lastPointer
      ) {
        const dx = e.clientX - lastPointer[0]
        const dy = e.clientY - lastPointer[1]
        ...
        if (draggingNode?.type === 'image') {
          setImages((p) =>
            p.map((i) =>
              i.id === draggingNode.id
                ? { ...i, x: i.x + dwx, y: i.y + dwy }
                : i,
            ),
          )
          ...
          setTexts((p) =>
            p.map((t) =>
              t.id === draggingNode.id
                ? { ...t, x: t.x + dwx, y: t.y + dwy }
                : t,
            ),
          )
          ...
          setStrokes((p) =>
            p.map((s, idx) =>
              idx === draggingNode.id
                ? s.map((pt) => [pt[0] + dwx, pt[1] + dwy, pt[2]] as Point)
                : s,
            ),
          )
        } else if (isDragging) {
    ...
    (
      ...
      if (activeTool === 'eraser') {
        ...
        if (type === 'image') {
          ...
          setConnections((p) =>
            p.filter((c) => c.from.id !== id && c.to.id !== id),
          )
        } else if (type === 'stroke')
          setStrokes((p) => p.filter((_, idx) => idx !== id))
        else if (type === 'text') {
          setTexts((p) => p.filter((x) => x.id !== id))
          setConnections((p) =>
            p.filter((c) => c.from.id !== id && c.to.id !== id),
          )
        }
      ...
      } else if (activeTool === 'arrow' && e.button === 0) {
        if (type === 'text' || type === 'image') {
          e.stopPropagation()
          const { x: wx, y: wy } = screenToWorld(e.clientX, e.clientY, camera)
          setActiveArrow({
  ...
  const cursorStyle = isSpacePanning
    ? isDragging
      ? 'grabbing'
      : 'grab'
    : isRightClickDragging
