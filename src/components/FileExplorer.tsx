...
export function FileExplorer({ isOpen, onClose }: FileExplorerProps) {
  ...
  const { data: items = [], isLoading: loading } = useQuery({
    queryKey: ['drive-items', currentFolder.id],
    queryFn: () => listFilesAndFolders(currentFolder.id),
    enabled: isOpen,
  })
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className="max-w-4xl h-[85vh] p-0 gap-0 overflow-hidden flex flex-col sm:rounded-3xl border-none shadow-2xl bg-white"
      ...
      >
        ...
        <div className="flex flex-col gap-4 p-6 border-b border-muted">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-2">
            <Button
              ...
              onClick={(e) => {
                e.stopPropagation()
                navigateTo({ id: 'root', name: 'Início' })
              }}
              onDragOver={(e) => {
                e.preventDefault()
            ...
            {currentFolder.id !== 'root' && (
              <>
                <ChevronRight className="h-4 w-4 text-muted-foreground/50" />
                <span className="text-sm font-bold px-3 py-1 bg-muted rounded-xl">
                  {currentFolder.name}
                </span>
              </>
            )}
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-6">
            ...
            {folders.map((f) => (
              ...
              >
                ...
                <span className="font-bold text-sm text-center line-clamp-2 px-2 leading-tight">
                  {f.name}
                </span>
                ...
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    setMenuItemId(menuItemId === f.id ? null : f.id)
                  }}
                >
                ...
                {menuItemId === f.id && (
                  <div
                    className="absolute top-10 right-2 bg-white rounded-xl shadow-xl border p-1 z-10 animate-in fade-in zoom-in-95"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <Button
                      variant="ghost"
                      className="w-full justify-start text-red-500 hover:text-red-600 hover:bg-red-50 rounded-lg"
            ...
            {boards.map((b) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    setMenuItemId(menuItemId === b.id ? null : b.id)
                  }}
                >
                  <MoreVertical className="h-4 w-4 text-muted-foreground" />
                </Button>
        ...
        <div className="p-4 bg-muted/30 border-t flex justify-between items-center text-xs font-semibold text-muted-foreground">
          ...
          <div className="flex gap-2">
            ...
            <span className="flex items-center gap-1">
              <FileJson className="h-3 w-3" /> Board
            </span>
          </div>
        </div>
      </DialogContent>
